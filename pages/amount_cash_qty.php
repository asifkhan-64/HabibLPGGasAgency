...
$getCartItems = mysqli_query($connect, "SELECT cart_tbl_qty.*,cart_tbl_qty.id AS cartID, qty_stock_purchase.*, categories.category_name, categories.stock_available, categories.sell_price FROM `cart_tbl_qty`
...
if (isset($_POST['makeInvoice'])) {
    ...
    for ($i = 0; $i < sizeof($arr_cart_id); $i++) {
        ...
        $updateQuery = mysqli_query($connect, "UPDATE cart_tbl_qty SET product_qty = '$product_qty', price = '$price' WHERE product_id = '$product_id' AND c_id = '$c_id' AND id = '$cart_id'");
    }
